#!/usr/bin/env bash
# Exercise the unreleased Dialog authorizer patch without rewriting this checkout.
set -euo pipefail

repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
dialog_revision=4c16de9e345d2b2d888d1008c5d3f0ca990c4807
patch_file="$repo_root/patches/dialog-connection-authorizer.patch"
prepare_only=false
...
if [[ ! -s "$patch_file" ]]; then
  echo "missing authorizer patch: $patch_file" >&2
  exit 2
fi
...
run_root=$(mktemp -d "${TMPDIR:-/tmp}/tonk-connection-authorizer.XXXXXX")
...
git -C "$run_root/dialog" apply --check "$run_root/tonk/patches/dialog-connection-authorizer.patch"
git -C "$run_root/dialog" apply "$run_root/tonk/patches/dialog-connection-authorizer.patch"
...
spike = tonk / "spikes/connection-policy.rs"
...
if any(test.get("name") == "connection_policy_spike" for test in consumer.get("test", [])):
    raise SystemExit("connection_policy_spike already registered; reassess harness injection")
# Register the unreleased API consumer only in the isolated copy. The ordinary
# checkout keeps building against its published, unchanged dependency contract.
consumer_manifest.write_text(consumer_text + '''
[[test]]
name = "connection_policy_spike"
path = "../../spikes/connection-policy.rs"
...
if [[ $# -eq 0 ]]; then
  set -- test -p tonk-access-service --features helpers --test connection_policy_spike
fi
cd "$run_root/tonk"
