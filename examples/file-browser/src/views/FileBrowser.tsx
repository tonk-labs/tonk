...
const FileBrowser: React.FC = () => {
  ...
  const [isCreatingDirectory, setIsCreatingDirectory] =
  ...
  const handleParentDirectory = () => {
    const parentPath = TonkService.getParentPath(currentPath);
  ...
  const handleDelete = async (path: string) => {
    if (window.confirm(`Are you sure you want to delete ${path}?`)) {
      try {
        await TonkService.removeItem(path);
        loadDirectory(currentPath);
      } catch (err) {
  ...
  const handleCreateDirectory = async () => {
    ...
    try {
      const path = TonkService.joinPath(currentPath, newItemName);
      await TonkService.createDirectory(path);
      setNewItemName('');
  ...
  const handleCreateFile = async () => {
    ...
    try {
      ...
      await TonkService.writeDocument(path, contentToSave);
      setNewItemName('');
      setFileContent('');
        ...
        <div className="bg-white rounded-2xl shadow-md p-6">
          ...
          {isCreatingDirectory && (
            <div className="mb-6 p-6 bg-[#f5f5f7] rounded-xl">
              <h3 className="text-xl font-medium mb-4 text-[#1d1d1f]">
                New Folder
              </h3>
              <div className="mb-4">
                <input
          ...
          <div className="rounded-xl overflow-hidden bg-[#f5f5f7]">
            {isLoading ? (
              ...
              <div className="p-8 text-center text-[#86868b]">
                <svg
                  className="mx-auto h-12 w-12 text-[#d2d2d7] mb-3"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
