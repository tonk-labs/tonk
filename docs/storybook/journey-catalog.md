...
| `ACCT-B03` | Submit an email that already owns an account, then recover. | Root missing; existing provider-free root; new add-account profile. | Whole browser test expects one orphaned credential, then success with another email. | Product decision on orphaning; restart and retry; concurrent availability race; local-root boundary. |
