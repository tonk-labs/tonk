...
| `ACCT-B03` | Enter an email and follow the ceremony it names. | Address known, unknown, malformed, or the service unreachable. | Browser test expects zero credentials across a taken address and an edit to a free one, through the registration dialog. | Same entry screen on the account panel, which still forks and can still mint an orphan; restart and retry; concurrent availability race; local-root boundary. |
