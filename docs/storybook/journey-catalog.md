...
| `ACCT-B03` | Enter an email and follow the ceremony it names. | Address known, unknown, malformed, or the service unreachable. | Browser tests expect zero credentials for a known address and exactly one for an unknown one. | Restart and retry; concurrent availability race; local-root boundary. |
...
| `UI-04` | Open a space home or explicit rendered route. | Blank/configured home; missing view/entity; local/remote; unauthorized. | Rendering/portal coverage outside `tonk-ui`; some E2E paths. | Top-level real-browser route matrix and interactive behavior, not only headless render. |
