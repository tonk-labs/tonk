...
| `ACCT-C02` | Start `tonk account login` and approve in an already-linked browser. | Default page; `--via`; `--no-open`; custom name; TTY/pipe. | Whole happy hybrid flows for callback/default/discovered provider. | OS-open failure, callback collision, timeout, malformed callback, restart after approval at every write. |
