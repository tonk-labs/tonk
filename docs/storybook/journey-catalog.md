...
| `ACCT-B02` | Choose Create and create a fresh account/passkey. | Root missing; provider-free root; add-account profile. | Whole happy path plus DOM form tests. | Passkey cancel/unavailable/PRF fallback, local save failure, remote timeout, response lost after commit, reload at every stage. |
| `ACCT-B03` | Enter an email and follow the ceremony it names. | Address known, unknown, malformed, or the service unreachable. | Browser test expects zero credentials across a taken address and an edit to a free one. | Restart and retry; concurrent availability race; local-root boundary. |
| `ACCT-B04` | Receive and open the activation email link. | Missing, damaged, valid, expired, already used link. | None in `activate.rs`. | Every branch, duplicate click, reload, offline, malformed body, receipt-report failure. |
| `ACCT-B05` | See activation pending and resend an activation email. | Customer absent, Registered, Active, Suspended, unreachable. | Partial component logic; happy activation used by broader browser tests. | Resend success/error/rate limit, stale email, Active idempotency, Suspended copy. |
...
| `ACCT-B11` | Load settings when account state is unconfigured or unhydrated. | `I2`, `I3`, provider online/offline. | Partial rendering/status tests. | Retry convergence, forbidden edits, reload loop, malformed descriptor, service return after offline. |
...
| `ACCT-B13` | Change the account display name. | Ready/unhydrated/revoked; Enter/blur; same/different value. | Partial DOM keyboard/blur tests. | Remote reject/conflict, duplicate Enter+blur, response loss, second-device concurrent rename, restart. |
| `ACCT-B14` | Add another passkey. | Active customer; missing/legacy passkey metadata; PRF/cancel/error. | Partial custody browser coverage. | Ceremony matrix, duplicate credential, fact/publish failure, reload and retry. |
...
| `ACCT-C02` | Start `tonk account login` and approve in an already-linked browser. | Default page; `--via`; `--no-open`; custom name; TTY/pipe; Safari cross-scheme warning. | Loopback bridge contract tests and whole happy hybrid flows for callback/default/discovered provider. | Real Safari/Chrome/Firefox bridge pass, OS-open failure, callback collision, timeout, malformed callback, restart after approval at every write. |
...
| ID | User journey and entry | Starting variants | Existing evidence | Missing or weak evidence |
| --- | --- | --- | --- | --- |
| `AUTH-01` | View devices and revoke another device in browser settings. | Self/other/unknown/already revoked; deep link. | Whole browser revoke-CLI flow; DOM list tests. | Offline, passkey cancel/wrong assertion, partial service publish, reload, concurrent revoke. |
...
| `AUTH-05` | Review and delete the whole account. | Zero/many owned spaces; joined spaces; multiple profiles/devices. | Whole browser account deletion/reuse-email flow. | Passkey denial, plan failure, response loss, partial space purge, restart, unrelated boundaries. |
...
| `SPACE-03` | Create a new local-only space while signed out. | Canonical/custom site; valid/invalid/colliding name. | Whole CLI process coverage; browser pre-activation flow covers local-first use. | Crash between data/registry/binding, read-only/full disk, concurrent same name. |
| `SPACE-04` | Create an account-owned space while signed in. | Customer Active/Registered/Suspended/offline; ready/unhydrated account. | Partial authority and browser flows. | All service states, ownership transaction crash, lost response, retry without duplicate space. |
...
| `SPACE-07` | Remove a space locally with confirmation. | TTY/no TTY; yes/no; owned/listed/local-only; data missing. | Broad process coverage. | Signal/crash during removal, failed partial filesystem delete, concurrent use/sync. |
...
| `COLLAB-03` | Join or claim an invite into a named space. | Fresh/existing name; remote/no remote; malformed/expired/revoked/already claimed. | Join, profile, authority tests; accountless claim-to-link flow. | Process error matrix, crash between claim and registration, response loss, concurrent claim. |
...
| `DATA-02` | Define/list a detail, directory, label, or title view. | Inline/file template; blank/existing home; `--home`. | Strong authoring/render coverage. | Template read failure, atomic home replacement under sync failure, route visibility in real browser. |
...
| `DATA-09` | Render a route to stdout or a file. | Directory/detail/explicit view; missing fields/view; output path errors. | Eight render integration tests. | Atomic output file, broken pipe, real-browser parity, concurrent source change. |
...
| `UI-04` | Open a space home or explicit rendered route. | Blank/configured home; missing view/entity; local/remote; unauthorized. | Rendering/portal coverage outside `tonk-ui`; Tonk-Prose list editing has deterministic marker, caret, and empty-item regressions; some E2E paths. | Top-level real-browser route matrix and hosted editor interaction, not only lower-layer or headless coverage. |
