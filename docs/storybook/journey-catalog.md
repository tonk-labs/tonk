...
| `ACCT-C13` | Retired: account-bound agent setup through join or browser-mediated connect. | Old sharing/account handoffs and legacy resume metadata. | Historical lower-layer evidence retained; new CLI refuses these entry points without mutation. | People and agents now use ACCT-C14. |
| `ACCT-C14` | Choose **connect a tool**, run one-way `tonk join` without browser/account approval, and revoke its grant group from Settings; **invite someone** remains browser-only membership. | Empty, same-account, unrelated-account, or malformed-legacy CLI; person link; repeated bearer holders; independent issuers/invites; fresh/returning space; profile/space switch; restart; offline or revoked authority. | Tool-only CLI routing/no-mutation, account-independence, credential isolation, worker explicit-space issuance, FAB source tests, two packaged whole-browser tool-connection journeys, and the existing packaged person-membership journey passed for Plan 005. | Profile switching while open, responsive/focus/reduced-motion inspection, published CLI/browser matrix, staging, Safari and global revocation propagation remain open. |
...
| `DATA-07` | Evaluate notation from inline text, file, explicit `-`, or piped stdin. | Query/write/mixed; JSON/quiet/home/dry-run/no-sync; file-relative `!include` (inline/stdin refuse it). | Strong notation/site coverage. | Broken pipe, signal during eval/sync, huge input, file mutation mid-read. |
