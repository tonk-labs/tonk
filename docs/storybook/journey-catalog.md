...
| `AUTH-05` | Review and delete the whole account. | Zero/many owned spaces; joined spaces; multiple profiles/devices. | Historical whole-browser deletion/reuse-email flow; current Settings and worker boundaries fail closed before a passkey or side effect. | Root-signed exact-plan authorization, whole-browser passkey denial, response loss, partial space purge, restart, unrelated boundaries. |
| `AUTH-06` | Recover after account deletion. | Deleted selected profile; another local profile; retained joined/local replicas. | Historical whole profile/email release path plus rotation unit behavior; current deletion is unavailable before dispatch. | Durable operation recovery, offline reload, second-device stale state, explicit status/error on next remote action. |
