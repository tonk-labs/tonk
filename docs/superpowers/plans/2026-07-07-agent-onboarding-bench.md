...
- AMENDED (post-Task-2 probe, second pass): in sandbox mode `EPISODE_PATH` is CONSTRUCTED (`/usr/bin:/bin:/usr/sbin:/sbin` + `EPISODE_BIN` prepend), never inherited — the harness shell's PATH carries `~/.cargo/bin`. nix-darwin's zshenv preserves an explicitly passed PATH whenever `__NIX_DARWIN_SET_ENVIRONMENT_DONE` is inherited (always true under the harness). Consequence: episode.sh resolves `timeout`/`claude`/`codex` to absolute paths via the harness PATH before invoking them.
