...
### Search increment (2026-09-24)

- The collection now searches the directory's rendered names and optional descriptions without opening spaces. It rechecks matches when bound text changes in place. A clear action restores the cards and focus to the search input; a no-match status appears only after a query against a nonempty directory. The create action stays available throughout.
- The first Chrome run exposed a latent registration gap: `<hub-collection>` rendered but its author element never upgraded, so its existing frame dealer was inert too. The collection now declares `with="main@profile:tonk"` for definition lookup. The browser test waits for the upgraded element before exercising search.
- After the final product change, 50 standard-library tests, the new real-Chrome search test (including an in-place description update and Clear focus), `cargo fmt --all -- --check`, the style-boundary test, and `git diff --check` pass. The focused profile-library reconciliation test passed after the collection context change, and the existing responsive Hub Chrome test passed before the final search focus adjustment. The reconciliation test required host access for its local socket. A stale worker test helper needed the optional `None` description argument to compile.
- Search is a local filter over the currently rendered directory. It makes no claim about a settled empty directory, account sync, real previews, activity time, Discover, or duplication; those contracts remain open above.

### Hub, account ceremony, and settings visual pass (2026-09-24)

- The browser comparison at 1440 × 900 exposed a centered header, undersized card and dialog, misplaced collection heading, and an oversized add-account ceremony. The Hub now uses the mock's full-width header, collection panel, larger cards, readable Gestalte headings, compact anchored account ceremony, and a blurred backdrop. The create and rename dialogs use the Hub appearance without changing the shared FABB dialog default.
- The `/account` route keeps the Hub roster behind the ceremony instead of showing the old settings page. The email-confirmation notice is suppressed on that route while the ceremony already supplies the instruction, so it cannot appear redundantly behind the popup. The `/settings` page presents separate details and passkeys panels, a page heading and back link, and no switch-account panel. Passkey creation seconds are formatted as a date. Agent access remains because it is a live capability rather than a mock fixture.
- At 390 × 844, the card keeps its horizontal preview/caption layout and the fixed navigation supplies the only visible create action; the redundant collection create tile is hidden. The production search field remains visible on mobile because it is a working feature. Discover, Duplicate, real preview imagery, and last-activity labels still require the production contracts described above.
- Verification after the account/settings changes: the local `tonk-ui-preview` build succeeded and `it_adds_an_account_from_the_hub_in_one_step` selected and passed 1/1 real-Chrome tests, including confirmation, settings geometry, absence of switch-account UI, and the formatted passkey date. After the final mobile rule, a fresh preview build and `it_renders_the_responsive_hub_collection` selected and passed 1/1 real-Chrome tests. Isolated Chrome screenshots at desktop and mobile were compared directly with `hub.html`; no runtime console errors appeared (only existing preload warnings). `cargo fmt --all -- --check`, `node --test rust/tonk-ui/tests/style-boundaries.test.mjs`, and `git diff --check` passed. Broader account/browser suites, Safari, devices, hosted deployment, and production-data preview/catalog behavior remain unverified.
...
### Continued mock alignment (2026-09-25)

- The user explicitly removed the Your spaces / Discover tabs and search from scope and then confirmed they should be removed from the Hub. Removed the visible collection heading, search controls, filtering controller, and obsolete search E2E case. The collection starts with cards; its accessible name and page heading remain.
- Matched the reference's light paper/collection palette, 107px desktop mark, desktop gutters, unruled header, caption spacing, inset action divider, staggered cards, mobile frame weights, compact mobile card action placement, navigation marks, More chevron, and separate right/bottom modal frame rules. The collection observer reconnects when its element reconnects.
- The new browser regression creates a named space with a description and checks desktop/mobile overflow, absence of the removed toolbar, independent card links/actions, 64px mobile previews, action alignment, and opening Rename. Existing responsive and stylesheet-delivery checks retain coverage.
- Static expectations for the old auto-fill grid/1250px content width and the pre-appearance removal-dialog tag were stale; updated them to the current explicit grid, 1166px content width within the reference's 1250px outer container, and shared Hub dialog appearance.
- Real preview imagery, last-activity data, truthful account-sync status, and independent duplication remain open contracts from the earlier increments. No fake metadata or inactive Duplicate action was added. Discover is excluded by the latest request.
- Validation: pending final focused gates. Development-server screenshots inspected at desktop, 390px mobile, mobile dark mode, and the mobile create dialog; the signed-out account-connections request returned 409, separately from the layout checks. No deployed or device result is implied.

### Account destination and settings correction (2026-09-25)

- The follow-up screenshots showed the retired `/account` menu and a missing Switch account panel. Linked account triggers now target `/settings`; old `/account` visits redirect there once link state is known and no registration ceremony is open. Unlinked profiles retain the existing add-account ceremony.
- Restored the fact-backed profile switcher and Add account entry on settings. Guarded switcher label writes so its MutationObserver does not continually rewrite its own subtree. Removed the retired menu pane.
- Moved the subscribed email field inside the display-name form, before Save changes, preserving native form ownership and command submission. Matched title/control sizing and tones, added the passkey key mark and sign-out arrow, and removed hidden dialog hosts from the panel grid. Agent access is a collapsed disclosure after Delete account; its API is queried only from linked settings.
- Extended the signup browser case to return home, click the linked name, assert `/settings`, verify Save changes remains associated with the name form below email, and verify the Switch account / Sign out row aligns with Your details / Passkeys.
- Final validation: 50/50 standard-library tests pass. The optimized Nix CLI, browser-test archive, and UI preview build successfully. Four selected real-Chrome tests pass (signup/name-to-settings and panel/form geometry, stylesheet delivery, mobile card actions/Rename, and responsive collection), with 97 unrelated cases skipped. The tested preview's `profile.yaml` SHA-256 matches the source (`96732333b1873f267abda9bc2075115072bbb60d1fb90999d22bb204ad7bbfa6`). Formatting, style-boundary checks, and patch hygiene pass.
- Test infrastructure: the default test-server launcher tried to rebuild another UI before browser startup. Superseded runs were interrupted before any test result. The successful run used the supported `TONK_UI_TEST_SERVER` override pointing at an existing Caddy launcher and explicitly set `TONK_UI_TEST_ARTIFACT` to the newly built Nix preview. No product workaround was added.
- The older multi-account test selectors now follow the direct settings route and `.switch-panel`; that broader test was not rerun. Safari/device coverage, hosted deployment, full screenshot parity across collection sizes, and the previously listed metadata/copy capabilities remain unverified or incomplete.

### Header, account ceremony, and Agent access correction (2026-09-25)

- Matched the header actions at 38px, centered them in the same row, and removed the New space outline that added two visible pixels.
- Matched Hub Add an account to the FABB task study: 360px shell, 25px corners, solid disc, 17px controls, 18px explanatory copy, continuous surface, inset input focus ring, and equal footer actions. Kept the real email/passkey ceremony and mobile positioning.
- Removed account switching and Add account from settings. Agent access now occupies the second-row left panel beside Sign out, with the existing data, refresh, and revoke behavior. Deleted the collapsed duplicate below Delete account. Reset panel margins to avoid the legacy Agent access offset.
- Updated the existing account-flow regression to check header alignment, modal width/radius, direct settings navigation, absence of switching, and Agent access placement. Profile lifecycle tests prepare/switch profiles through the existing API now that those controls are absent.
- Initial schema check: 48/50 passed; two obsolete assertions required the removed roster controls and were updated. Initial browser run caught an 18px offset using a stale served profile asset; the source margin fix was already present. Final checks are recorded below.
- Final verification: 50/50 standard-library tests passed; the updated real-Chrome account-flow test passed against the final dev assets (profile source/dist SHA-256 matched `3ea903d948b8a34cf17fc949ac80057b3da1805fd7b0e0758fbf2e831babd543`). Header geometry, rounded popup, direct settings route, and Agent access/Sign out alignment all passed. Desktop and 390px mobile popup screenshots inspected; final mobile bounds are x=16..374, bottom=762 within 390x844, with left-aligned Cancel. Formatting, stylesheet boundary test, and whitespace check passed. The three adapted profile-lifecycle tests compiled but were not rerun; Safari/device and hosted deployment remain unverified.

### FABB menu and duplicate creation follow-up (2026-09-25)

- Removed the visible Connect a tool menu action, preserving the separate underlying tool-connection contract.
- Read the reported live Hub card destinations: the two same-named cards have distinct repository DIDs. This is duplicate creation, not duplicate rendering of one repository. Existing spaces are retained.
- Creation investigation: the collection view nests inside the Hub view, and both resolve the bubbling space-create submit binding. Added a browser regression that submits the collection card once and counts newly created space identities.
- Reproduced with the collection-card browser regression: one submit created two distinct repository IDs. Fixed `on/space-create` with `stop-propagation: true`, so the nearest view dispatches once instead of letting the enclosing Hub dispatch the same event again. The same regression passed against the final served profile asset (source/dist SHA-256 `0a3ef606b18da076ea2533624f7a13289b480265ded269b2150820069e625aa0`).
- Focused FABB markup test passed with the tool action absent. Initial browser fixture selector matched a hidden rename field; scoped the test to `.stack > space-create` before reproducing and fixing the actual bug. Existing duplicate repositories were not deleted. Hosted and Safari verification were not run.
