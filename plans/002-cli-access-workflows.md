...
## Agent theme and signed-out terminal linking follow-up

Checkpoint before this work: `cafa4861c` commits the two-workflow simplification, dev setup and terminal picker refinements. Generated replica directories remain untracked.

- Agent prompt buttons used fixed light-only FABB colors on normal themed surfaces. Core and regenerated onboarding agent views now use theme-aware Web Awesome text/on-ink colors; the copy focus ring contrasts with the surrounding surface. Actual Web Awesome copy-button shadow DOM preview was measured in light and dark themes, and the dark 320px layout inspected.
- Login and signup completion previously unconditionally navigated home. The registration dialog now captures only the local `/settings/link#tonk-terminal-v1=...` request, including query/fragment, before authentication; completion returns there. If the overlay left the URL unchanged, a route event refreshes the signed-in picker. Other completion destinations remain home. External destinations are rejected by the return-path helper.
- The empty picker offers a create-space link in a separate tab, keeping approval open for refresh after creation. Regression coverage includes destination filtering, browser dialog closure/navigation, and showing/hiding this empty-state action.
- This follow-up has not yet been committed. Real passkey signup/login against the local service has not been rerun end to end; targeted checks are recorded below when complete.
- Final focused checks pass: two Wasm registration return-route/dialog-close tests, one Wasm picker/empty-action test, and the scoped-agent standard-library regression. Rust formatting and whitespace checks pass. Existing worker dead-code warnings remain in the test build; no full Clippy or live passkey journey is claimed for this follow-up.
