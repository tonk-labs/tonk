...
//!
//! For PUT requests, optional integrity verification:
//! - X-Checksum-SHA256: <base64-sha256> - If provided, R2 will verify upload integrity
...
async fn handle_put_inner(
    ...
    // 5. Extract SHA256 checksum for R2 integrity verification
    let checksum = parse_checksum_header(req)?;
...
/// Parse X-Checksum-SHA256 header if present.
///
/// The header value must be base64-encoded SHA256 (32 bytes).
/// Returns None if header is not present, error if malformed.
fn parse_checksum_header(req: &Request) -> std::result::Result<Option<Checksum>, ServiceError> {
    let header = req
        .headers()
        .get("X-Checksum-SHA256")
        .map_err(|e| ServiceError::internal(format!("Header read error: {}", e)))?;

    let Some(checksum_b64) = header else {
        return Ok(None);
    };
    ...
    let checksum_bytes = base64::engine::general_purpose::STANDARD
        .decode(&checksum_b64)
        .map_err(|e| {
            ServiceError::invalid_base64(format!("Invalid base64 in X-Checksum-SHA256: {}", e))
        })?;

    let checksum_array: [u8; 32] = checksum_bytes.try_into().map_err(|_| {
        ServiceError::invalid_argument("X-Checksum-SHA256 must be exactly 32 bytes (SHA256)")
    })?;

    Ok(Some(Checksum::Sha256(checksum_array)))
}
...
///
/// Returns headers the client must include when following the redirect via
/// X-Required-Headers response header (JSON array of [name, value] pairs).
fn redirect_307(
    url: &str,
    ...
    // If there are required headers (e.g., checksum), include them for the client
    if !required_headers.is_empty() {
        let headers_json = serde_json::to_string(required_headers)
            .map_err(|e| ServiceError::internal(format!("JSON serialize failed: {}", e)))?;
        headers
            .set("X-Required-Headers", &headers_json)
            .map_err(|e| ServiceError::internal(format!("Header set failed: {}", e)))?;
    }
