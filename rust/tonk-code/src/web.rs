...
/// Inject the `<tonk-code>` JS module into `<head>` exactly once.
///
...
/// pass `/tonk-code.js` if you've used the matching `copy-file`
/// link in `index.html`. Routing the URL through the caller (rather
/// than hardcoding it here) keeps deployment-path concerns in one
/// place: whichever crate owns `index.html` decides where assets
/// live.
...
const SCRIPT_ID: &str = "tonk-code-loader";
