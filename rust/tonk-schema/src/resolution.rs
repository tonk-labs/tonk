...
// -----------------------------------------------------------------
// Environment — a live source the resolution surface runs against.
// -----------------------------------------------------------------

/// A live environment the resolution surface can run against.
///
/// Resolution always needs both halves of a query: a [`Source`]
/// (where facts live) and a [`QueryEnv`] (the capabilities to read
/// them). A host that opens a branch holds both; a caller that is
/// host-agnostic — notably the language server, which cannot name
/// a concrete branch or operator — holds neither directly. This
/// trait is the seam: the host pairs source and env behind it, and
/// the agnostic caller drives resolution through these methods.
///
/// [`SourceEnv`] is the implementation for every `(Source,
/// QueryEnv)` pair, built via [`source_env`].
#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
pub trait Environment {
    /// Resolve a [`ConceptReference`] to its definition.
    async fn resolve_concept(
        &self,
        reference: ConceptReference,
    ) -> Result<Option<ConceptDefinition>, ResolveError>;

    /// Resolve an [`AttributeReference`] to its definition.
    async fn resolve_attribute(
        &self,
        reference: AttributeReference,
    ) -> Result<Option<AttributeDefinition>, ResolveError>;

    /// Resolve a published name to the entity it points at.
    async fn resolve_named_entity(&self, name: &str) -> Result<Option<Entity>, ResolveError>;

    /// Every concept the environment holds, fully resolved.
...
/// A live environment built from a [`Source`] and the [`QueryEnv`]
/// its operations take. The host constructs one per request and
/// hands it to an agnostic caller as an [`Environment`].
pub struct SourceEnv<'a, Env> {
    source: Source<'a>,
    env: &'a Env,
}

impl<'a, Env> SourceEnv<'a, Env> {
    /// Borrow the underlying source.
    pub fn source(&self) -> Source<'a> {
        self.source.clone()
    }

    /// Borrow the underlying query env.
    pub fn query_env(&self) -> &'a Env {
        self.env
    }
}

/// Pair a source and a query env into an [`Environment`].
pub fn source_env<'a, Env>(source: impl Into<Source<'a>>, env: &'a Env) -> SourceEnv<'a, Env> {
    SourceEnv {
        source: source.into(),
        env,
    }
...
#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl<Env: QueryEnv + ConditionalSync> Environment for SourceEnv<'_, Env> {
    async fn resolve_concept(
        &self,
        reference: ConceptReference,
    ) -> Result<Option<ConceptDefinition>, ResolveError> {
    ...
    async fn list_concepts(&self) -> Result<Vec<ConceptDefinition>, ResolveError> {
        ConceptDefinition::list(self.source.clone())
            .perform(self.env)
            .await
    }

    async fn list_names(&self) -> Result<Vec<Name>, ResolveError> {
        NamedReference::list(self.source.clone())
            .perform(self.env)
            .await
    }
}
