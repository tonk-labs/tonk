...
7. Add a WAF rate limiting rule for unauthenticated request creation (zone `tonk.xyz`,
   ...
   http.request.uri.path in {"/codes" "/accounts/preflight"})`,
...
- **Rate rule** (zone `tonk.xyz`, and the staging host): one rate-limiting
  ...
  bootstrap paths. Each rule's expression should match `/codes` and
  `/accounts/preflight`:
  ...
  Verify each environment's existing rule covers both paths in the
  expression.
...
WAF → Rate limiting rules) and that its path list covers both bootstrap
paths.
