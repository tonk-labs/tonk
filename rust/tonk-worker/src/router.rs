...
mod authorize;
pub use authorize::{AuthorizeResponse, StatusResponse, authorize, status};
...
pub mod tests {
    ...
    -> (Space<ServiceWorkerStorageBackend>, Operator, Delegation) {
        ...
        let space_keypair = Operator::from_passphrase("test tonk space").await;
        let space_did = space_keypair.did().to_string();

        // Generate operator keypair
        let operator = Operator::from_passphrase("test tonk operator").await;
