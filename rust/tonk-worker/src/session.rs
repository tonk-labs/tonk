...
mod tests {
    ...
    /// The dual of the presign test above. A presign SHOULD be
    /// session-bounded (this device is acting right now), but an
    /// invite's membership delegation is handed to someone who claims it
    /// hours or days later, so the mint MUST NOT ride the presign's
    /// operator claim. Today it doesn't: `Delegate::perform` binds the
    /// PROFILE's signer and proves only to the profile, yielding
    /// `space -> profile -> membership` with no session material and no
    /// expiry. The two claims live one seam apart, and nothing else
    /// fails if a refactor reroutes the mint through the presign path —
    /// the links would just start dying within [`SESSION_TTL_SECONDS`]
    /// (12h) of minting. This pins the durable shape. (The chain is
    /// still PROFILE-anchored — revoking the minting device orphans its
    /// invites; anchoring at the account instead is open design work.)
    #[dialog_common::test]
    async fn it_mints_membership_delegations_that_outlive_the_session() {
        let (storage, profile) = scratch().await;
        let session = open(&profile, &storage).await.unwrap();

        // The `space -> profile` grant space creation persists.
        let space = Ed25519Signer::generate().await.unwrap();
        let grant = DelegationBuilder::new()
            .issuer(dialog_credentials::Signer::from(space.clone()))
            .audience(&profile.did())
        ...
        // The exact mint `create_invite` performs for a share.
        let membership = Ed25519Signer::generate().await.unwrap();
        let delegation: UcanDelegation = profile
            .access()
            .claim(dialog_capability::Subject::from(space.did()).attenuate(dialog_effects::Use))
            .delegate(membership.did())
            .perform(&session.operator)
            .await
            .unwrap();

        let operator = session.operator.did();
        for hop in delegation.into_chain().proofs() {
            assert_ne!(
                hop.issuer(),
                &operator,
                "an invite hop must be issued by a durable principal, \
                 never the session-scoped operator"
            );
            if let Some(expiration) = hop.expiration() {
                assert!(
                    expiration.to_unix() > session.expires_at,
                    "an invite hop expiring at {} dies with the operator \
                     session ({}) — the link stops proving within \
                     SESSION_TTL of minting",
                    expiration.to_unix(),
                    session.expires_at,
                );
            }
