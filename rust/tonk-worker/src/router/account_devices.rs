//! Devices as facts in the account space.
//!
...
use tonk_common::log;
use tonk_worker_api::{AccountSummary, PasskeyMetadata};
...
/// A device the approving browser just authorized, to be described in the
/// account space under this profile's account.
// Only the browser's device-authorization ceremony registers one.
#[cfg_attr(
...
/// browser runs this because it is the side that MINTS the grant: minting
...
/// grant.
/// Answers the grant's attachment id — the delegation CID it describes.
///
...
mod tests {
    ...
    async fn it_lists_this_device_for_a_profile_with_a_root() {
        ...
        let own = state.profile.did().to_string();
        crate::router::account_state::describe_own_device(&state).await;
        let devices = local_devices(&state).await.unwrap();
        assert_eq!(devices.len(), 1);
        assert_eq!(devices[0].1, own);
    }
    ...
    /// device links serve it back from those facts — the whole loop without an
    ...
    async fn it_registers_a_device_as_account_space_facts() {
        ...
        let devices = local_devices(&*state.read().await).await.unwrap();
        let (registered, _) = devices
            .iter()
            .find(|(_, did)| *did == device.did().to_string())
            .expect("the registered device is listed");
        assert_eq!(registered.title.0, "e2e terminal");
    }
}
