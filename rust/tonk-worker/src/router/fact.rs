...
pub async fn assert_fact(
    ...
    let entity: Entity = path.entity
        .parse()
        .map_err(|e| TonkWorkerError::Internal(format!("Invalid entity '{}': {}", path.entity, e)))?;
    ...
    // Transact the relation
    {
        ...
        space.transact([relation]).await.map_err(|e| {
    ...
    // Return response immediately - push will happen in background
