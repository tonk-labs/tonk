//! `slide remote add` / `slide remote list` / `slide remote
...
//! concepts so tonk-ui's existing read paths see slide-created
...
/// Name of the meta branch every slide repo carries alongside
/// `main`. Matches the worker's `META_BRANCH` so tonk-ui sees
/// slide's writes without configuration.
pub const META_BRANCH: &str = "meta";

/// Default remote name slide auto-registers when joining an
...
/// One row of `slide remote list` — also the shape `find`
/// returns and `slide invite --remote` consumes when embedding
...
pub struct UpstreamOutcome {
    ...
    /// [`crate::site::BRANCH_NAME`] for slide today).
...
/// Failure modes for the slide remote API.
#[derive(Debug, Error)]
pub enum RemoteError {
    /// `set_upstream` was given a remote name that's not on the
    /// meta branch.
    #[error("remote '{0}' is not registered; add it with `slide remote add` first")]
...
/// repo whose DID differs from the local one — slide uses this
...
pub async fn list(site: &SlideSite) -> Result<Vec<RemoteRecord>, RemoteError> {
    ...
    for row in rows {
        let endpoint = match decode_endpoint(&row.address) {
            Some(url) => url,
            // Skip non-UCAN addresses — slide only knows how to
...
pub async fn find(site: &SlideSite, name: &str) -> Result<Option<RemoteRecord>, RemoteError> {
...
/// implicit-create-on-first-write semantics mean a fresh slide
/// site doesn't need a separate meta-branch bootstrap step.
async fn open_meta(site: &SlideSite) -> Result<Branch, RemoteError> {
    site.repository
        .branch(META_BRANCH)
        .open()
        .perform(&site.operator)
