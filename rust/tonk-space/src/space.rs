...
impl<Backend: PlatformBackend + 'static> Space<Backend> {
    ...
    pub async fn resolve_remote_branch(
        ...
        let mut remote_branch = remote_site.repository(repo_did).branch(branch_name);
...
impl CredentialsInfo {
    /// Create credentials info from dialog-s3-credentials Credentials
    pub fn from_credentials(credentials: &dialog_s3_credentials::Credentials) -> Self {
        match credentials {
            ...
            dialog_s3_credentials::Credentials::Ucan(ucan_creds) => {
