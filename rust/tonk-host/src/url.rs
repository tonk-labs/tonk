...
/// document with when the entity is not one yet; `heads` (space-
/// separated) selects a past version to read. `None` when the route
...
pub(crate) fn document_url(
    ...
    let mut separator = '?';
    for (name, value) in [("format", format), ("heads", heads)] {
