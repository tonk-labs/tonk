...
///
/// A sealed guest cannot dispatch an event into its parent document. When the
/// portal bridge is present, relay one unprivileged notification outward;
/// every nested parent repeats this function until the trusted top document
/// dispatches the event owned by `index.html`.
pub fn announce_update() {
    let Some(win) = window() else { return };
    if let Ok(tonk) = Reflect::get(&win, &JsValue::from_str("tonk"))
        && !tonk.is_null()
        && !tonk.is_undefined()
        && let Ok(update) = Reflect::get(&tonk, &JsValue::from_str("updateAvailable"))
        && let Ok(update) = update.dyn_into::<Function>()
        && update.call0(&tonk).is_ok()
    {
        return;
    }
...
mod request_header_tests {
    ...
    #[dialog_common::test]
    fn it_relays_a_stale_build_announcement_out_of_a_sealed_guest() {
        let window = web_sys::window().expect("browser window");
        let previous = Reflect::get(&window, &JsValue::from_str("tonk")).unwrap();
        let calls = Rc::new(Cell::new(0));
        let observed = calls.clone();
        let update = Closure::<dyn FnMut()>::new(move || observed.set(observed.get() + 1));
        let bridge = Object::new();
        Reflect::set(
            &bridge,
            &JsValue::from_str("updateAvailable"),
            update.as_ref(),
        )
        .unwrap();
        Reflect::set(&window, &JsValue::from_str("tonk"), &bridge).unwrap();

        announce_update();

        if previous.is_undefined() {
            Reflect::delete_property(&window, &JsValue::from_str("tonk")).unwrap();
        } else {
            Reflect::set(&window, &JsValue::from_str("tonk"), &previous).unwrap();
        }
        assert_eq!(
            calls.get(),
            1,
            "an iframe-local DOM event cannot reach the trusted top prompt"
        );
    }
