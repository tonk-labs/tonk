...
Tonk is a tiny CLI for reading and writing tonk/dialog facts via the
asserted-notation DSL (`rust/tonk-notation/guide.md`). Its entire mutating
surface is one command, `tonk eval`, that consumes a notation document and
...
better. Tonk is the redesign that takes the notation seriously.
...
- Carry depends on the full sync stack (`tonk-invite`,
  `dialog-remote-ucan-s3`, `tonk-access-service`). Tonk doesn't, so its
  build is cheaper and its blast radius smaller.

## Goals

1. **Single learnable surface.** An agent reading
   `tonk-notation/guide.md` plus `tonk --help` should be able to use the
   tool without reading any other file.
2. **One round trip per task.** A query + the asserts that depend on its
   bindings + a retraction should all fit in one `tonk eval` call, exactly
...
- Multi-replica / sync / invite minting. Use carry or the future tonk
  CLI for that. If tonk needs networked data later, we'll reach for
...
- A service mode. Tonk is a one-shot per invocation.

## CLI surface

```
tonk eval [-c "<notation>"] [<file>] [-]   # the only mutating/reading verb
tonk init [<label>]                         # create .tonk/ in $PWD
tonk identity [--reset]                     # show or regen the local DID
tonk guide                                  # print tonk-notation/guide.md
tonk schema                                 # print branch's attrs+concepts as a notation doc
tonk migrate [--from <path>] [--move]       # one-shot .carry/ -> .tonk/ migration
```
...
| Form                         | Source            |
|------------------------------|-------------------|
| `tonk eval -c "<doc>"`      | inline string     |
| `tonk eval <file>`          | file path         |
| `tonk eval -` or `tonk eval` (no args, with piped stdin) | stdin |
...
There is no `--branch` flag at v0. Tonk writes to `main` only.
...
There is no `--repo <path>` flag at v0. Tonk walks up from `$PWD`
...
`tonk init` defensively at the start of a task.)
...
this repo can still learn the notation by running `tonk guide`.

### `tonk schema`

Reads every attribute and concept asserted on the branch and emits a
notation document that, if re-submitted to `tonk eval` against an
...
`Directory::At(.carry)`. Tonk will use the *same* layout under
`Directory::At(.tonk)`. So byte-for-byte, the dialog data inside
`.carry/` and the dialog data tonk expects inside `.tonk/` are the
same shape.
...
3. **Copy** `.carry/` → `.tonk/`. (Default. With `--move`, tonk
...
4. **Open** the new `.tonk/` to verify the dialog repository loads
   and both branches are intact. If open fails, tonk rolls back
   (deletes the partially-copied `.tonk/`) and reports the error.
5. **Print** the repo DID and a one-line note that any sync remotes
   configured under carry's meta branch are preserved (tonk doesn't
...
data dir already and is shared by carry and tonk. The DID stays the
same; the agent doesn't need to re-delegate anything.

The migration is intentionally not automatic on first `tonk init`:
silently moving the user's data on a defensive `tonk init` would
...
This is the question to push back on: should tonk expose more
fine-grained commands (`tonk assert <concept> field=value …`,
`tonk query <concept> field=?` etc.) like carry did, or should it
...
2. **One DSL is cheaper to teach than two.** With `eval`, the agent
   reads `tonk guide` once and is done. Anything it can do, it does
...
3. **LLMs write YAML well.** Token-for-token, agents are more reliable
   ...
   `tonk assert person name="Alice" age=28 @alice` invocations,
...
5. **It mirrors what's already proven.** `tonk-worker`'s `/evaluate`
   ...
   and the editor (`tonk-ui`) drives it. Tonk is the same idea,
   stripped of the HTTP envelope.

The cost is real: a one-off `tonk eval -c "person: name: \"Alice\""`
is wordier than `tonk query person name=Alice` would be. But agents
...
subcommand. We can add `tonk guide --topic <name>` later if that
...
out of tonk is the same language it writes into tonk. That has a
...
3. **One grammar to learn.** With JSON output, the agent has to
   ...
   notation output, there is one grammar and `tonk guide` covers
   all of it.
...
For `--quiet`, tonk emits only the envelope.
...
Reusing the worker's type verbatim means a future "tonk as a thin
...
└── src/
    ...
    ├── schema_cmd.rs               # `tonk schema` introspection
...
plan-and-commit loop, and the response shaping. Tonk should not
...
`tonk-evaluator` crate)** so that both the worker route and `tonk
...
- Tonk's `eval` becomes: read source → call the same
  `evaluator::run` → render with the chosen formatter.

This refactor is the only non-trivial cross-crate change tonk
requires; the rest is plumbing. We can do tonk v0 by *copying* the
relevant code into `tonk/src/eval.rs` and only extracting later if a
...
output type — tonk and worker stay schema-compatible by construction.
...
into a shared module so tonk and the worker share one evaluator.
Worker keeps passing tests. No new behaviour.

**Phase 1 — `tonk eval` against a local repo.** New `rust/tonk/`
...
`tonk eval -c '<query>'` output back into another `tonk eval`.

**Phase 2 — agent-friendliness polish.** `tonk guide`, `tonk
schema`, `tonk migrate`, `--dry-run`, `--quiet`. Add a snippet in
...
relational schema and query it") and measure:
   ...
   - tool calls per task,
   - whether the agent uses `tonk schema` / `tonk guide`
     unprompted.
...
1. **Should `tonk schema` round-trip to a runnable document, or just
   ...
   `tonk schema`'s output too. Probably worth it; flag for review.

2. **Do we want `tonk eval --watch <file>`?** Re-run on file change.
...
3. **Telemetry.** Carry has a telemetry sidecar. Tonk should ship
   without one and not add it without a specific motivating use case.

4. **Ergonomic shortcut for `tonk eval -c "<doc>"`.** SQL CLIs let you
   pipe the doc on stdin without `-c`. Tonk already does that. The
   open question is whether `tonk "<doc>"` (no subcommand, treat first
