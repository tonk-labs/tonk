...
pub fn concept_entity(space_did: &str, concept_name: &str) -> Result<Entity> {
...
/// is returned.
pub fn qualify_attribute(concept_name: &str, key: &str) -> Result<String> {
...
pub fn short_attribute(concept_name: &str, attr: &str) -> String {
...
pub async fn open_session(
    ctx: &SpaceContext,
) -> Result<Session<dialog_artifacts::replica::Branch<FsBackend, SigningAuthority>>> {
    let backend = FsBackend::new(&ctx.storage_path).await?;
    let authority = SigningAuthority::from(&ctx.operator);
    let replica = Replica::open(authority, ctx.space_did.clone().into(), backend)?;
...
pub async fn open_branch(
    ctx: &SpaceContext,
) -> Result<dialog_artifacts::replica::Branch<FsBackend, SigningAuthority>> {
    let backend = FsBackend::new(&ctx.storage_path).await?;
    let authority = SigningAuthority::from(&ctx.operator);
    let replica = Replica::open(authority, ctx.space_did.clone().into(), backend)?;
    let branch_id = BranchId::new("main".to_string());
