...
Scope: bring tonk to feature-parity with carry's sync surface so
the agent-driven CLI and the human-driven tonk-ui browser can
read and write the same data through a shared remote.

## Goal

Tonk is the headless interface; tonk-ui is the human one. Both
target the same dialog repository — tonk writes locally to
...
Tonk and tonk-ui hold **different DIDs**, not the same one.
Their profile keypairs live in separate physical stores: tonk
...
1. The minter (tonk *or* tonk-ui) creates a repository and
...
Tonk therefore needs `tonk-invite` (mint + claim) plus the
...
tonk remote list
tonk remote set-upstream <remote>            # links main → <remote>/main

tonk eval [--no-sync] …                       # auto pull-before / push-after by default
tonk push                                    # pushes main to its upstream
tonk pull                                    # pulls main from its upstream
tonk status                                  # synced / ahead / behind / diverged / no-upstream
```

Auto-sync: a committing `tonk eval` pulls its upstream in before
evaluating and pushes the resulting commit back out afterward,
when an upstream is configured. Opt out per-invocation with
`--no-sync`, or for a whole session with the `TONK_NO_SYNC`
...
already committed, so the manual `tonk push` / `tonk pull` flow
...
- **`join`** parses an invite URL, claims it (redelegating to
  ...
  After `tonk join`, `tonk pull` works without further
  configuration.
...
- **`remote set-upstream`** sets `main`'s upstream to
  `<remote>/main` (tonk is single-branch) and writes a
...
- **`push` / `pull`** call dialog's `Branch::push()` /
  `Branch::pull()` directly. Tonk has no subscriptions, so
...
the agent ↔ browser handoff being load-bearing. Tonk's binary
size grows, but the dep graph still excludes
`tonk-access-service` and the broader sync-service plumbing
(tonk is purely a *client* of access services, never a host).

## Meta-branch interop

When tonk creates a remote, sets an upstream, or claims an
...
1. The browser-side worker's `GET /api/repository/{name}` reads
   these concepts to populate the UI. Without tonk's writes,
   tonk-ui wouldn't know the remote exists even after a pull.
2. Future tonk invocations (or another tool) discover existing
   remotes by querying the meta branch — no separate registry.
3. After `tonk join`, the meta branch records what the claim
...
`tonk-worker::router::join`'s post-claim wiring. Tonk
extracts the small subset it needs into `tonk::remote` and
`tonk::invite` rather than depending on the worker.
...
2. **Mint-and-claim round trip** — tonk site A mints an invite,
   tonk site B parses it and claims. Verifies the chain
   serializes/deserializes correctly and the claimer's DID
   ends up with delegated authority on A's subject. No remote
   needed.

3. **Remote-as-meta** — `tonk remote add` writes a `Remote`
   concept, `tonk remote list` reads it back. These don't
...
`tonk::sync` exposing `push` and `pull` over a `TonkSite`. CLI
subcommands. Tests use the in-process upstream pattern. No new
crate dependencies.

**Phase 2a — invite mint + claim.** Add `tonk-invite` plus its
transitive UCAN deps. New module `tonk::invite` with `mint` and
`claim` functions over a `TonkSite`. CLI subcommands `tonk
invite` and `tonk join`. Tests cover the round-trip plus
...
and the rest of the dep block. New module `tonk::remote`
...
error, plus a `tonk doctor`-style check that confirms the
...
- **Audience-scoped invites.** The mint path stays
  ...
  flow is solid.
- **`tonk remote remove`.** Worker doesn't expose a remove
