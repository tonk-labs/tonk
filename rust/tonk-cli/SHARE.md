...
The agent runs tonk locally, generates one URL, the human pastes it into
...
1. **Concept share** — `tonk share concept <name>` produces a URL that
   lands on tonk-ui's auto-render at `/space/<repo>/branch/main/concept/<name>`
   (PR #445). Phase-1 demo target.

2. **View share** — `tonk share view <name>` produces a URL that lands
...
- **Option A.** Tonk ships the seed as part of `tonk schema` output
  for any new repo, but does not auto-write it on `tonk init`. The
  agent is told (via the guide) "if you want HTML views, paste this
  block first."
- **Option B.** `tonk init --views` writes the seed schema as part of
  bootstrap. One extra commit at init time; nothing to memorise.

Lean A for v0 — keeps `tonk init` pure. Revisit if agents trip on
forgetting to seed.
...
and `then=` are appended by tonk. Reasoning:

- `tonk-invite` should stay agnostic — it's about delegation, not
  post-claim navigation. The launcher parameters live in tonk and in
...
```
tonk views                                    # list view-shaped entities
tonk concepts                                 # list named concepts on the meta branch
tonk share concept <name>  [--ui-base <url>]  # push + invite + URL
tonk share view   <name|entity> [--ui-base <url>]
```

Reasoning for each:

- **`tonk views`** — read-only. Joins `view ?e: body: ?html` against
  `dialog.meta/name ?e: ?name`. Prints `<name>  <entity>  <body-bytes>`
  in three columns. No new write paths.

- **`tonk concepts`** — read-only. Lists user-defined concepts on the
  meta branch by name. 90% overlap with `tonk schema` already; this
  is a slimmer single-purpose listing.

- **`tonk share concept <name>`** — pushes the local repo to its
  upstream (calls `tonk::sync::push` if the upstream is set; errors if
  not), mints an audience-open invite over the local repo (calls
  `tonk::invite::mint` with the configured remote), assembles a URL
  with `&then=branch/main/concept/<name>`, prints it.

- **`tonk share view <name|entity>`** — same shape as `share concept`,
...
This is a separate PR against tonk-ui; tonk can ship its share commands
...
2. **Share-URL composition** — given a fixture `view! my-list: …`,
   `tonk share view my-list --ui-base https://example.test/join`
   prints a URL that:
...
3. **`tonk views` / `tonk concepts`** — listing tests against a
...
3. **`tonk share` without an upstream.** What happens if the local
   repo has no remote configured? Two choices: (a) refuse with a
   pointer to `tonk remote add`, (b) silently mint an invite without
...
- **Phase 1 — concept share.** `tonk share concept <name>`,
  `tonk concepts` (read), tonk-ui `&then=` PR. Demo target: the
  worked example from PR #445's description (a `person` concept
  rendered live via `/concept/person`).

- **Phase 2 — view share.** Seed-schema documentation in `tonk guide`,
  `tonk share view`, `tonk views`. Demo target: agent asserts a
