...
/// the spot yet. The spinner is right because the state is genuinely
...
pub(crate) fn guest_content(site: &str) -> String {
    format!(
        "<tonk-display entity='{site}' model='tonk:site'>\
         ...
         <div slot='no-model' hidden class='site-pending'><wa-spinner></wa-spinner></div>\
         </tonk-display>"
    )
}
...
mod tests {
    ...
    /// just does not have the spot; slotted, it is a spinner that heals
