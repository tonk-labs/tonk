...
pub fn TonkShell() -> impl IntoView {
    ...
    // Initialize the space: wait for SW, check status, setup upstream if needed
    let init_resource = LocalResource::new(|| async {
        ...
        // If upstream is not configured, set it up automatically
        if !status.has_upstream {
            log!("No upstream configured, setting up sync...");
            api::authorize().await?;
            log!("Upstream sync configured successfully");
        }
