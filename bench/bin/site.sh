#!/usr/bin/env bash
# Register the per-run tonk spot and wire it to the local access
# service: spot new, remote add, set-upstream. The episode agent then
# works inside $RUN_DIR/site with everything pre-connected.
#
# The spot is named by TONK_SPOT (exported by run.sh) and its site
# lives at $RUN_DIR/site, adopted via `--site` so the run directory
# stays self-contained rather than the CLI's canonical spots/ root.
# Every `tonk` call here resolves through TONK_SPOT — the CLI never
# consults the cwd, so no `cd` is involved or useful.
#
# Env: ROOT, RUN_DIR, BENCH_URL, TONK_SPOT
set -euo pipefail
...
setup() {
  # `tonk spot new` prints "DID: did:key:..." — the repository's subject
  ...
  # as SPACE_NAME.
  new_out="$("$TONK" spot new "$SPOT" --site "$SITE")"
  printf '%s\n' "$new_out"
  ...
  # `tonk spot new` commits the seed before the upstream exists, so
