    ...
    event.respondWith(
      (async () => {
        ...
        if (referrer) {
          ...
          // Extract app name if referrer is from /apps/[app-name]/
          const appMatch = referrerPath.match(/^\/apps\/([^\/]+)\//);
          if (appMatch) {
        ...
        // Handle app paths: /apps/[app-name]/...
        if (pathname.startsWith('/apps/')) {
          // If path ends with / or is just the app name, append index.html
          if (pathname.endsWith('/')) {
            pathname = pathname + 'index.html';
          } else if (!pathname.includes('.', pathname.lastIndexOf('/'))) {
            // If no file extension after the last slash, assume it's a directory
            pathname = pathname + '/index.html';
          }
          ...
          if (!target) {
            return new Response(
              `App path "${pathname}" not found. Make sure the app is properly loaded.`,
              {
                status: 404,
                headers: { 'Content-Type': 'text/plain' },
              }
        ...
        if (appContext && !pathname.startsWith('/apps/')) {
          // Rewrite the path to include the app context
          const rewrittenPath = `/apps/${appContext}${pathname}`;
          console.log(`Rewriting ${pathname} to ${rewrittenPath}`);

          const target = await readDoc(rewrittenPath);
