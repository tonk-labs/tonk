...
pub trait RandomAccess: Read + Write + Seek + Send + std::fmt::Debug {
    ...
    fn position(&mut self) -> Result<u64> {
        self.stream_position().map_err(|e| anyhow::anyhow!("Failed to get position: {}", e))
    }
    
    /// Seek to a specific position
    fn seek_to(&mut self, pos: u64) -> Result<()> {
        self.seek(SeekFrom::Start(pos)).map_err(|e| anyhow::anyhow!("Failed to seek to position {}: {}", pos, e))?;
        Ok(())
    }
    
    /// Read exact number of bytes at current position
    fn read_exact_at(&mut self, buf: &mut [u8]) -> Result<()> {
        self.read_exact(buf).map_err(|e| anyhow::anyhow!("Failed to read exact bytes: {}", e))
    }
    
    /// Write bytes at current position
    fn write_at(&mut self, data: &[u8]) -> Result<()> {
        self.write_all(data).map_err(|e| anyhow::anyhow!("Failed to write bytes: {}", e))
    }
...
impl PathTreeNode {
    ...
    fn insert(&mut self, path_components: &[String], full_path: String) {
        ...
        let child = self.children.entry(component.clone()).or_insert_with(PathTreeNode::new);
...
impl<R: RandomAccess> Bundle<R> {
    ...
    fn read_entry_data(&mut self, metadata: &EntryMetadata) -> Result<Option<Vec<u8>>> {
        ...
        let mut file = archive.by_name(&metadata.path)
    ...
    pub fn get_prefix(&mut self, prefix: &[String]) -> Result<Vec<(Vec<String>, Vec<u8>)>> {
        ...
        let entries: Vec<EntryMetadata> = self.index.get_prefix_entries(&prefix_path)
            .into_iter()
            .cloned()
            .collect();
...
mod tests {
    ...
    fn create_complete_test_bundle() -> Result<Vec<u8>> {
        ...
        zip_writer.start_file("misc/subfolder/hidden_message.txt", SimpleFileOptions::default())?;
    ...
    fn test_load_bundle_without_manifest() {
        ...
        assert!(result.is_err(), "Expected error when loading bundle without manifest.json");
        let error = result.unwrap_err();
        assert!(error.to_string().contains("manifest.json not found in bundle"));
    }
    ...
    fn test_load_bundle_with_manifest() {
        ...
        assert_eq!(manifest.network_uris[1], "wss://realtime.example.com/socket");
    ...
    fn test_manifest_version_validation() {
        ...
        let zip_data = create_invalid_manifest_bundle().expect("Failed to create invalid manifest bundle");
        
        // Try to load the bundle - should fail due to unsupported manifest version
        let result = Bundle::from_bytes(zip_data);
        assert!(result.is_err(), "Expected error for unsupported manifest version");
        
        let error = result.unwrap_err();
        assert!(error.to_string().contains("Unsupported manifest version: 2"));
    }
    ...
    fn test_read_root_files() {
        ...
        let welcome_data = bundle.get(&["welcome.txt".to_string()])
            .expect("Failed to read file")
            .expect("File not found");
        assert_eq!(String::from_utf8(welcome_data).unwrap(), "Hello from the root directory!");
        
        // Test reading readme.txt
        let readme_data = bundle.get(&["readme.txt".to_string()])
            .expect("Failed to read file")
            .expect("File not found");
        assert_eq!(String::from_utf8(readme_data).unwrap(), "This is a sample collection of text files.");
    }
    ...
    fn test_seek_to_nested_files() {
        ...
        let report_data = bundle.get(&["documents".to_string(), "report.txt".to_string()])
            .expect("Failed to read file")
            .expect("File not found");
        assert_eq!(String::from_utf8(report_data).unwrap(), "Quarterly results look promising.");
        
        // Test seeking to notes/todo.txt
        let todo_data = bundle.get(&["notes".to_string(), "todo.txt".to_string()])
            .expect("Failed to read file")
            .expect("File not found");
        assert_eq!(String::from_utf8(todo_data).unwrap(), "Remember to water the plants.");
        
        // Test seeking to deeply nested file
        let nested_data = bundle.get(&["misc".to_string(), "subfolder".to_string(), "nested.txt".to_string()])
            .expect("Failed to read file")
            .expect("File not found");
        assert_eq!(String::from_utf8(nested_data).unwrap(), "Deep inside the folder structure!");
    }
    ...
    fn test_random_access_seeking() {
        ...
        let files_to_test = vec![
            (vec!["misc".to_string(), "subfolder".to_string(), "hidden_message.txt".to_string()], "You found the secret message!"),
            (vec!["notes".to_string(), "ideas.txt".to_string()], "Build something amazing today!"),
            (vec!["documents".to_string(), "summary.txt".to_string()], "Executive summary complete."),
            (vec!["misc".to_string(), "data.txt".to_string()], "Random data goes here."),
        ];
        
        for (key, expected_content) in files_to_test {
            let data = bundle.get(&key)
                .expect("Failed to read file")
                .expect(&format!("File not found: {:?}", key));
    ...
    fn test_prefix_queries() {
        ...
        let docs = bundle.get_prefix(&["documents".to_string()])
            .expect("Failed to get prefix");
        assert_eq!(docs.len(), 2);
        
        // Get all files under misc/subfolder
        let subfolder = bundle.get_prefix(&["misc".to_string(), "subfolder".to_string()])
            .expect("Failed to get prefix");
        ...
        for (_key, data) in subfolder {
            ...
            assert!(content == "Deep inside the folder structure!" || 
    ...
    fn test_nonexistent_file() {
        ...
        let result = bundle.get(&["nonexistent.txt".to_string()])
            .expect("Failed to read file");
    ...
    fn test_bundle_from_bytes() {
        ...
        let data = bundle.get(&["welcome.txt".to_string()])
            .expect("Failed to read file")
            .expect("File not found");
        assert_eq!(String::from_utf8(data).unwrap(), "Hello from the root directory!");
    }
    ...
    fn test_simple_put_in_memory() {
        ...
        bundle.put(key.clone(), content.clone()).expect("Failed to put simple file");
        
        // Read it back
        let read_content = bundle.get(&key)
    ...
    fn test_put_and_get_new_file() {
        ...
        bundle.put(new_key.clone(), new_content.clone()).expect("Failed to put new file");
        
        // Re-read the file to verify it was added correctly
        let read_content = bundle.get(&new_key)
            .expect("Failed to read new file")
            .expect("New file not found");
        
        assert_eq!(read_content, new_content);
        assert_eq!(String::from_utf8(read_content).unwrap(), "This is a newly added file!");
    ...
    fn test_put_multiple_files_and_read_back() {
        ...
        let files_to_add = vec![
            (vec!["test1.txt".to_string()], b"Content of test file 1".to_vec()),
            (vec!["documents".to_string(), "new_report.txt".to_string()], b"New quarterly report data".to_vec()),
            (vec!["notes".to_string(), "urgent.txt".to_string()], b"Urgent reminder!".to_vec()),
        ];
        
        // Add all files
        for (key, content) in &files_to_add {
            bundle.put(key.clone(), content.clone()).expect("Failed to put file");
        }
    ...
    fn test_put_and_prefix_query() {
        ...
        let new_files = vec![
            (vec!["new_dir".to_string(), "file1.txt".to_string()], b"File 1 content".to_vec()),
            (vec!["new_dir".to_string(), "file2.txt".to_string()], b"File 2 content".to_vec()),
            (vec!["new_dir".to_string(), "subdirectory".to_string(), "file3.txt".to_string()], b"File 3 content".to_vec()),
        ];
        ...
        let new_dir_files = bundle.get_prefix(&["new_dir".to_string()])
            .expect("Failed to get prefix");
    ...
    fn test_put_duplicate_path_behavior() {
        ...
        let original_content = bundle.get(&key)
            .expect("Failed to read original file")
            .expect("Original file not found");
        assert_eq!(String::from_utf8(original_content).unwrap(), "Hello from the root directory!");
        ...
        assert!(result.is_err(), "Expected error when adding duplicate filename");
        let error = result.unwrap_err();
        ...
        let still_original = bundle.get(&key)
            .expect("Failed to read original file")
            .expect("Original file not found");
        assert_eq!(String::from_utf8(still_original).unwrap(), "Hello from the root directory!");
    }
    ...
    fn test_put_with_file_data_source() {
        ...
        temp_file.write_all(&zip_data).expect("Failed to write test data to temp file");
