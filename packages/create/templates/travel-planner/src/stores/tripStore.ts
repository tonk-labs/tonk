...
import { Trip, Location, CalendarEvent, PlaceSuggestion, SharedNote, TripMember } from '../types/travel';
...
interface TripState {
  ...
  // Place suggestions
  addSuggestion: (suggestion: Omit<PlaceSuggestion, 'id' | 'suggestedAt' | 'votes' | 'status'>) => void;
  voteSuggestion: (suggestionId: string, userId: string) => void;
  approveSuggestion: (suggestionId: string) => void;
  rejectSuggestion: (suggestionId: string) => void;
  
  // Shared notes
  addNote: (note: Omit<SharedNote, 'id' | 'createdAt' | 'lastModifiedAt'>) => void;
...
export const useTripStore = create<TripState>(
  sync(
    (set) => ({
      ...
      removeMember: (memberId) => {
        set((state) => ({
          currentTrip: state.currentTrip ? {
            ...state.currentTrip,
            members: state.currentTrip.members.filter(m => m.id !== memberId)
          } : null
        }));
      },
      ...
      removeEvent: (eventId) => {
        set((state) => ({
          currentTrip: state.currentTrip ? {
            ...state.currentTrip,
            events: state.currentTrip.events.filter(e => e.id !== eventId)
          } : null
        }));
      },
    ...
    }),
    {
      ...
      initTimeout: 30000,
      onInitError: (error) => console.error('Trip store sync initialization error:', error)
    }
  )
);