...
Sprinkles runs both the Tonk relay server and host-web frontend simultaneously using [mprocs](https://github.com/pvolok/mprocs), giving you a split-screen terminal UI to monitor both processes.
...
Just publish `@tonk/host-web` and `@tonk/relay` to npm, update versions, and `pnpm install` will pull from the registry.
