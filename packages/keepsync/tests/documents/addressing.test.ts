...
describe('Document Addressing', () => {
  ...
  it('test setup has a valid root document', async () => {
    ...
    const otherRootHandle = await repo.find(rootId);
    const rootDoc = otherRootHandle.doc();
  ...
  it('it can get and set a document', async () => {
    ...
    if (docNode) {
      ...
      const doc = docNode.doc() as { text: string };
  ...
  it('it will delete a document deeply', async () => {
    ...
    const result = await traverseDocTree(repo, rootId, '/test/this/deeply/nested/doc');
    expect(result).toBeDefined();
    expect(result?.targetRef?.pointer).toBe(id);

    // Delete the entire /test directory
    await removeDocument(repo, rootId, '/test');

    // Verify the document path no longer exists
    const deletedResult = await traverseDocTree(repo, rootId, '/test/this/deeply/nested/doc');
  ...
  it('fixes duplicate directory bug with hierarchical docIds', async () => {
    ...
    if (appResult!.node.children!.length === 1 && appResult!.node.children![0].name === 'app') {
      ...
      if (nestedAppChild.pointer) {
        const nestedAppHandle = await repo.find(nestedAppChild.pointer);
        const nestedAppDoc = nestedAppHandle.doc();
