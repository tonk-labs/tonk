...
export async function handleMessage(message: Message): Promise<void> {
  ...
  switch (msgType) {
    ...
    case 'getServerUrl':
      await handleGetServerUrl({ id: msgId as string, launcherBundleId: effectiveBundleId });
      break;

    case 'getManifest':
      await handleGetManifest({ id: msgId as string, launcherBundleId: effectiveBundleId });
      break;
    ...
    case 'toBytes':
      await handleToBytes({ id: msgId as string, launcherBundleId: effectiveBundleId });
      break;

    case 'forkToBytes':
      await handleForkToBytes({ id: msgId as string, launcherBundleId: effectiveBundleId });
      break;
