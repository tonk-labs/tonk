...
export function setConnectionHealth(launcherBundleId: string, healthy: boolean): void {
  const state = bundleStates.get(launcherBundleId);
  if (state?.status === 'active') {
    bundleStates.set(launcherBundleId, { ...state, connectionHealthy: healthy });
  }
}
...
export function incrementReconnectAttempts(launcherBundleId: string): number {
  ...
  if (state?.status === 'active') {
    ...
    bundleStates.set(launcherBundleId, { ...state, reconnectAttempts: newAttempts });
    return newAttempts;
  }
  return 0;
}
...
export function setHealthCheckInterval(launcherBundleId: string, interval: number | null): void {
  const state = bundleStates.get(launcherBundleId);
  if (state?.status === 'active') {
    bundleStates.set(launcherBundleId, { ...state, healthCheckInterval: interval });
  }
}
...
export function getWatcherEntries(launcherBundleId: string): [string, { stop: () => void }][] {
  const state = bundleStates.get(launcherBundleId);
  if (state?.status === 'active') {
