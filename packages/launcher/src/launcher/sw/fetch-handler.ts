...
import { getActiveBundleState, getBundleState, getInitializationPromise } from './state';
...
export function handleFetch(event: FetchEvent): void {
  ...
  if (isKnownRuntimeFile || (isRuntimePath && (hasBundleIdParam || isRuntimeFont))) {
  ...
  if (parsed && url.origin === location.origin && !isRootRequest) {
    ...
    event.respondWith(
      (async () => {
        try {
          ...
          if (initPromise && (!bundleState || bundleState.status !== 'active')) {
          ...
          if (!tonkInstance) {
            logger.error('Tonk not initialized for bundle - cannot handle request', {
              launcherBundleId,
    ...
    logger.debug('Ignoring fetch - not a valid /space/<bundleId>/<appSlug>/ request', {
