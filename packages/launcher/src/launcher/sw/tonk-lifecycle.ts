...
export async function autoInitializeFromCache(): Promise<void> {
  try {
    ...
    if (!launcherBundleId) {
      logger.debug('No launcher bundle ID found in cache, waiting for initialization message');
      return;
    }
    ...
    const storageConfig = { type: 'indexeddb' as const, namespace: launcherBundleId };
...
export async function loadBundle(
  ...
  try {
    ...
    if (existingState?.status === 'loading') {
      logger.debug('Bundle is currently loading, waiting...', { launcherBundleId });
    ...
    const loadStorageConfig = { type: 'indexeddb' as const, namespace: launcherBundleId };
