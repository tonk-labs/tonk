...
export async function performHealthCheck(launcherBundleId: string): Promise<boolean> {
...
export async function attemptReconnect(launcherBundleId: string): Promise<void> {
  const activeBundle = getActiveBundleState(launcherBundleId);
  if (!activeBundle) {
  ...
  if (attempts >= MAX_RECONNECT_ATTEMPTS) {
    if (continuousRetryEnabled) {
      ...
      logger.error('Max reconnection attempts reached', { launcherBundleId, attempts });
  ...
  await postResponse({ type: 'reconnecting', launcherBundleId, attempt: attempts });
...
export async function reestablishWatchers(launcherBundleId: string): Promise<void> {
...
export function startHealthMonitoring(launcherBundleId: string): void {
  const activeBundle = getActiveBundleState(launcherBundleId);
  if (!activeBundle) {
    logger.warn('Cannot start health monitoring: no active bundle', { launcherBundleId });
    return;
  }
  ...
  const interval = setInterval(async () => {
    ...
    if (!isHealthy && currentBundle.connectionHealthy) {
      ...
      logger.warn('Connection lost, starting reconnection attempts', { launcherBundleId });
