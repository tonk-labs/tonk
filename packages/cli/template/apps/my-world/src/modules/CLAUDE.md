# DO NOT EDIT - AUTO-GENERATED FROM docs/src/llms/shared/modules.md

# This file is automatically generated from the documentation.

# Edit the source file instead: docs/src/llms/shared/modules.md

# DocVersion: e4a56ca (2025-08-11)

# How to create a Module

Encapsulate business logic and handle IO operations (like API calls, file operations, etc.). Note
that stores already synchronized data into a DB, so this is only IO _outside_ of our application —
ie. 3rd party services. It may also encapsulate non-IO business logic that doesn't involve rendering
UI components or storing application state (e.g. utilities, imperative dom logic, etc).
...
- Be focused on a single Module — Export all the Module logic through the Module interface —
...
const validateAuth = () => {
  ...
  if (!authHeaders) {
    throw new AuthenticationError(
      'No Google OAuth token available. User may need to authenticate.'
    );
  }
  return authHeaders;
};
...
export const createEventFn = createFunction(
  ...
  async (event: Omit<CalendarEvent, 'id'>): Promise<CalendarEvent> => {
    ...
    try {
      return await callGoogleCalendarApi<CalendarEvent>(endpoint, 'POST', event);
    } catch (error) {
      if (error instanceof AuthenticationError) {
        throw error; // Re-throw authentication errors
      }
...
export const updateEventFn = createFunction(
  ...
  async (
    ...
    try {
      return await callGoogleCalendarApi<CalendarEvent>(endpoint, 'PUT', updatedFields);
    } catch (error) {
      if (error instanceof AuthenticationError) {
        throw error; // Re-throw authentication errors
      }
...
}>([configureCalendarFn, getEventsFn, createEventFn, updateEventFn, deleteEventFn]);
