...
Implemented on 2026-09-14. Profile reconciliation now validates the current claims, atomically replaces recorded definitions and provenance, repairs attributable legacy fields, caches only a validated branch revision, and runs at boot and after both account pull paths. Native and Wasm fixture suites cover recorded, legacy, conflict-retry, hydration, ready-account, profile-switching, and no-op behavior. Real Chrome scenarios cover a persisted worker upgrade, a warm offline restart and reconnect, and repair after a second account writer republishes stale claims without clearing browser state.

The mixed-version limit remains: an active old writer can publish stale claims again, and a current client repairs them on its next successful lifecycle sweep. The browser evidence is Chrome-only; Safari and deployed-host behavior have not been verified.

Completion means the fixture-backed reconciliation and account lifecycle tests pass, the persisted-browser upgrade passes without storage reset, unchanged sweeps perform no library writes, and legacy/mixed-version limits are stated. It does not mean every historical unowned definition has been removed or every deployed browser has already updated.
